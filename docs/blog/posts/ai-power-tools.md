...
If I try to build that chair relying solely on the speed of my tools rather than a fundamental understanding of carpentry, I am more likely to end up with an ugly, uncomfortable, and unsafe piece of furniture. There's a good chance it will collapse the moment someone sits on it.
...
To return to the workshop analogy, there are times when you absolutely must put down the power tools. If you are cutting a delicate dovetail joint, you do not reach for a power saw. You reach for a chisel. Where mistakes are costly, speed is the wrong metric.

The same principle applies to software. There are critical areas of our codebases where an AI's broad-brush, statistically average approach lacks the necessary finesse. When implementing intricate, high-stakes domain logic, a predictive model left to its own devices is a liability. While an LLM might perfectly scaffold the API endpoint for you, it should not be relied upon to interpret the nuances of complex rules. It just knows which words usually sit next to each other.

## The Feature Factory Fallacy

The concern is that it is incredibly easy to be impressed by the sheer speed at which AI generates _"functional"_ code. This creates a dangerous illusion that syntax is all there is to it, masking the reality of why we must build so much infrastructure around the code itself.
...
AI doesn't create these organisational challenges but can amplify them if they already exist.
...
These guardrails were always important before AI; they are critical now.
...
- **"The _next_ model will be an agent that can reason!"** Ah, the tech equivalent of [_"jam tomorrow"_](https://en.wikipedia.org/wiki/Jam_tomorrow). Throwing another trillion parameters at a model makes it a better interpolator, but it does not magically grant it the ability to sense the unwritten business rules of your domain. It will simply predict the syntax with even greater false confidence. The human must still verify if that syntax represents the _truth_.
...
For experienced engineers, the upside is obvious if you understand what the tool is built for. We should not ignore power tools just because they can be dangerous in the wrong hands.
