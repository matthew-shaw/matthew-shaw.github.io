...
3. **_Muri_ (Overburden)**: As the root cause, _Muri_ is the accumulation of cognitive and technical load on people and systems. Sustainable performance requires a sociotechnical approach that integrates both team design and system health.

The paper concludes by highlighting common industry anti-patterns that institutionalise all three forces, and acts as a practical diagnostic framework. Genuine, sustainable improvement depends not on new frameworks, but on systematically reducing overload, stabilising flow, and empowering teams to deliver small, safe, continuous changes.
